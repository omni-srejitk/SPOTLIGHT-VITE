...
export const Carousal = ({ data }) => {
  ...
  return (
    <div className=" no-scrollbar m-2 flex h-60 w-[95%] justify-start  gap-4 overflow-x-auto bg-[black] p-2 text-[1.05rem] text-[black]">
      <div className="relative z-10 h-full min-w-[70%] rounded-xl border-0 bg-[#FAE77D] text-center outline-none">
        ...
        <div className="ml-12 mt-8 mb-4 flex h-1/4 w-4/5 flex-row items-center justify-start">
          {/* Todo: => image changes */}
          <div className="z-10 flex h-16 w-20 items-center justify-center rounded-[2rem] border-[1px] border-black bg-white ">
        ...
        <p className="relative z-10 px-4">{slicedArray[0]}.</p>
      ...
      {modifiedStory.map((str) => {
        return (
          <div key={str} className=" relative h-full min-w-[70%] ">
            <div className="absolute top-0 right-0 z-0 w-auto">
              <img src="/images/Union.svg" />
            </div>
