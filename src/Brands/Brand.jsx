...
const Brand = () => {
  ...
  const brandName = String(brand?.brandName?.slice(0,1)).toUpperCase() + brand?.brandName?.slice(1)
    ...
    return useQuery(
      ...
      () => {
        return axios.get(
          ...
          `https://engine.omniflo.in/api/method/omniflo_lead.omniflo_lead.api.frappe_api.customer_profile?brand=${encodeURI(BRAND)}`
        ,{
          headers:{
      ...
      {
        select: (data) => data?.data?.message?.filter((store)=> 
          
          store.latitude!== null || store.longitude!==null?
           {latitude : store.latitude,longitude: store.longitude} : false
        ),
      }
    );
  };
